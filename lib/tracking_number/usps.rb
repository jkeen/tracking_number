module TrackingNumber
  ...
  class USPS91 < USPS
    SEARCH_PATTERN = [/(\b(?:420\s*\d{5})?9\s*[134]\s*(?:(?:[0-9]\s*){20}\b))/, /(\b([0-9]\s*){20}\b)/]
    VERIFY_PATTERN = /^(?:420\d{5})?(9[134][0-9]{19})([0-9])$/

    # Sometimes these numbers will appear without the leading 91, 93, or 94, though, so we need to account for that case

    def decode
      # Application ID: 91, 93, or 94
    ...
    def matches
      if self.tracking_number =~ /^(420\d{5})?9[134]/
